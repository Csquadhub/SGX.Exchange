...
export function useStakedGmxSupply(library, active) {
  const gmxAddressArb = getContract(ARBITRUM, "SGX");
  ...
  const gmxAddressAvax = getContract(AVALANCHE, "SGX");
...
export function useTotalGmxInLiquidity() {
  ...
  let totalSGX = useRef(bigNumberify(0));

  const { data: gmxInLiquidityOnArbitrum, mutate: mutateSGXInLiquidityOnArbitrum } = useSWR(
    [`StakeV2:gmxInLiquidity:${ARBITRUM}`, ARBITRUM, getContract(ARBITRUM, "SGX"), "balanceOf", poolAddressArbitrum],
    {
      fetcher: fetcher(undefined, Token),
    }
  );
  const { data: gmxInLiquidityOnAvax, mutate: mutateSGXInLiquidityOnAvax } = useSWR(
    [`StakeV2:gmxInLiquidity:${AVALANCHE}`, AVALANCHE, getContract(AVALANCHE, "SGX"), "balanceOf", poolAddressAvax],
    {
      fetcher: fetcher(undefined, Token),
    }
  );
  const mutate = useCallback(() => {
    mutateSGXInLiquidityOnArbitrum();
    mutateSGXInLiquidityOnAvax();
  }, [mutateSGXInLiquidityOnArbitrum, mutateSGXInLiquidityOnAvax]);

  if (gmxInLiquidityOnAvax && gmxInLiquidityOnArbitrum) {
    let total = bigNumberify(gmxInLiquidityOnArbitrum).add(gmxInLiquidityOnAvax);
    totalSGX.current = total;
  }
  return {
    avax: gmxInLiquidityOnAvax,
    arbitrum: gmxInLiquidityOnArbitrum,
    total: totalSGX.current,
    mutate,
  };
}
...
function useGmxPriceFromArbitrum(library, active) {
  ...
  const gmxPrice = useMemo(() => {
    if (uniPoolSlot0 && ethPrice) {
      ...
      const gmxAddress = getContract(ARBITRUM, "SGX");
