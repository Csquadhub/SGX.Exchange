...
const VEST_WITH_SGX_ARB = "VEST_WITH_SGX_ARB";
const VEST_WITH_SGXLP_ARB = "VEST_WITH_SGXLP_ARB";
const VEST_WITH_SGX_AVAX = "VEST_WITH_SGX_AVAX";
const VEST_WITH_SGXLP_AVAX = "VEST_WITH_SGXLP_AVAX";
...
export default function ClaimEsSGX({ setPendingTxns }) {
  ...
  const esGmxIouAddress = getContract(chainId, "ES_SGX_IOU");
  ...
  const arbVesterAdddresses = [getContract(ARBITRUM, "GmxVester"), getContract(ARBITRUM, "SgxLpVester")];
  const avaxVesterAdddresses = [getContract(AVALANCHE, "GmxVester"), getContract(AVALANCHE, "SgxLpVester")];
  ...
  let stakingToken = "staked SGX";

  const shouldShowStakingAmounts = false;

  if (selectedOption === VEST_WITH_SGX_ARB && arbVestingData) {
  ...
  if (selectedOption === VEST_WITH_SGXLP_ARB && arbVestingData) {
    const result = getVestingValues({
  ...
  if (selectedOption === VEST_WITH_SGX_AVAX && avaxVestingData) {
    const result = getVestingValues({
      minRatio: bigNumberify(4),
      amount,
  ...
  if (selectedOption === VEST_WITH_SGXLP_AVAX && avaxVestingData) {
    const result = getVestingValues({
      minRatio: bigNumberify(320),
      amount,
      ...
      <div className="Page-title-section mt-0">
        ...
        {isArbitrum && (
          <div>
            <div className="Page-description">
              <br />
              <Trans>You have {formatAmount(esGmxIouBalance, 18, 2, true)} esSGX (IOU) tokens.</Trans>
              <br />
              <br />
              <Trans>The address of the esSGX (IOU) token is {esGmxIouAddress}.</Trans>
              <br />
              <Trans>
                The esSGX (IOU) token is transferrable. You can add the token to your wallet and send it to another
              ...
              <br />
              <Trans>
                After claiming, the esSGX tokens will be airdropped to your account on the selected network within 7
                days.
              </Trans>
              <br />
              <Trans>The esSGX tokens can be staked or vested at any time.</Trans>
              <br />
              <Trans>
                Your esSGX (IOU) balance will decrease by your claim amount after claiming, this is expected behaviour.
              </Trans>
              <br />
              <Trans>
            ...
            <div className="ClaimEsGmx-vesting-options">
              <Checkbox
                ...
                isChecked={selectedOption === VEST_WITH_SGX_ARB}
                setIsChecked={() => setSelectedOption(VEST_WITH_SGX_ARB)}
              >
                <div className="ClaimEsGmx-option-label">
                  <Trans>Vest with SGX on Arbitrum</Trans>
                </div>
                <img src={arbitrumIcon} alt="arbitrum" />
              </Checkbox>
              <Checkbox
                className="arbitrum btn btn-primary btn-left btn-lg"
                isChecked={selectedOption === VEST_WITH_SGXLP_ARB}
                setIsChecked={() => setSelectedOption(VEST_WITH_SGXLP_ARB)}
              >
                <div className="ClaimEsGmx-option-label">
                  <Trans>Vest with SGXLP on Arbitrum</Trans>
                </div>
                <img src={arbitrumIcon} alt="arbitrum" />
              </Checkbox>
              <Checkbox
                className="avalanche btn btn-primary btn-left btn-lg"
                isChecked={selectedOption === VEST_WITH_SGX_AVAX}
                setIsChecked={() => setSelectedOption(VEST_WITH_SGX_AVAX)}
              >
                <div className="ClaimEsGmx-option-label">
                  <Trans>Vest with SGX on Avalanche</Trans>
                </div>
                <img src={avaIcon} alt="avalanche" />
              </Checkbox>
              <Checkbox
                className="avalanche btn btn-primary btn-left btn-lg"
                isChecked={selectedOption === VEST_WITH_SGXLP_AVAX}
                setIsChecked={() => setSelectedOption(VEST_WITH_SGXLP_AVAX)}
              >
                <div className="ClaimEsGmx-option-label avalanche">
                  <Trans>Vest with SGXLP on Avalanche</Trans>
                </div>
                <img src={avaIcon} alt="avalanche" />
              </Checkbox>
            </div>
            ...
            {!error && (
              <div className="muted">
                <Trans>
                  You can currently vest a maximum of {formatAmount(maxVestableAmount, 18, 2, true)} esSGX tokens at a
                  ratio of {formatAmount(currentRatio, 4, 2, true)} {stakingToken} to 1 esSGX.
                </Trans>
                ...
                <Trans>
                  ...
                  esSGX at a ratio of {formatAmount(nextRatio, 4, 2, true)} {stakingToken} to 1 esSGX.
                </Trans>
