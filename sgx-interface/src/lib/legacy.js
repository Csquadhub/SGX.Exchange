...
export const SGXLP_DECIMALS = 18;
export const SGX_DECIMALS = 18;
...
export const SGXLP_COOLDOWN_DURATION = 15 * 60;
...
export const REFERRAL_CODE_KEY = "SGX-referralCode";
...
export const platformTokens = {
  42161: {
    ...
    SGX: {
      ...
      symbol: "SGX",
      decimals: 18,
      address: getContract(ARBITRUM, "SGX"),
    ...
    SGXLP: {
      ...
      symbol: "SGXLP",
      decimals: 18,
      address: getContract(ARBITRUM, "StakedSgxLpTracker"), // address of fsSGXLP token because user only holds fsSGXLP
      imageUrl: "https://github.com/gmx-io/gmx-assets/blob/main/SGX-Assets/PNG/SGXLP_LOGO%20ONLY.png?raw=true",
    },
  },
  43114: {
    // avalanche
    SGX: {
      ...
      symbol: "SGX",
      decimals: 18,
      address: getContract(AVALANCHE, "SGX"),
      imageUrl: "https://assets.coingecko.com/coins/images/18323/small/arbit.png?1631532468",
    },
    SGXLP: {
      name: "SGX LP",
      symbol: "SGXLP",
      decimals: 18,
      address: getContract(AVALANCHE, "StakedSgxLpTracker"), // address of fsSGXLP token because user only holds fsSGXLP
      imageUrl: "https://github.com/gmx-io/gmx-assets/blob/main/SGX-Assets/PNG/SGXLP_LOGO%20ONLY.png?raw=true",
    },
  },
};
...
export function getBuySgxLpToAmount(fromAmount, swapTokenAddress, infoTokens, glpPrice, usdgSupply, totalTokenWeights) {
...
export function getSellSgxLpFromAmount(toAmount, swapTokenAddress, infoTokens, glpPrice, usdgSupply, totalTokenWeights) {
  const defaultValue = { amount: bigNumberify(0), feeBasisPoints: 0 };
...
export function getBuySgxLpFromAmount(toAmount, fromTokenAddress, infoTokens, glpPrice, usdgSupply, totalTokenWeights) {
  ...
  fromAmount = adjustForDecimals(fromAmount, SGXLP_DECIMALS, fromToken.decimals);
...
export function getSellSgxLpToAmount(toAmount, fromTokenAddress, infoTokens, glpPrice, usdgSupply, totalTokenWeights) {
  const defaultValue = { amount: bigNumberify(0) };
  if (!toAmount || !fromTokenAddress || !infoTokens || !glpPrice || !usdgSupply || !totalTokenWeights) {
    return defaultValue;
  }
...
export function getStakingData(stakingInfo) {
  ...
  const keys = ["stakedGmxTracker", "bonusGmxTracker", "feeGmxTracker", "stakedSgxLpTracker", "feeSgxLpTracker"];
  const data = {};
  const propsLength = 5;
...
export function getProcessedData(
  ...
  data.glpPrice =
    data.glpSupply && data.glpSupply.gt(0)
      ? aum.mul(expandDecimals(1, SGXLP_DECIMALS)).div(data.glpSupply)
      : bigNumberify(0);

  data.glpSupplyUsd = supplyData.glp.mul(data.glpPrice).div(expandDecimals(1, 18));

  data.glpBalance = depositBalanceData.glpInStakedSgxLp;
  data.glpBalanceUsd = depositBalanceData.glpInStakedSgxLp.mul(data.glpPrice).div(expandDecimals(1, SGXLP_DECIMALS));

  data.stakedSgxLpTrackerRewards = stakingData.stakedSgxLpTracker.claimable;
  data.stakedSgxLpTrackerRewardsUsd = stakingData.stakedSgxLpTracker.claimable.mul(gmxPrice).div(expandDecimals(1, 18));

  data.feeSgxLpTrackerRewards = stakingData.feeSgxLpTracker.claimable;
  data.feeSgxLpTrackerRewardsUsd = stakingData.feeSgxLpTracker.claimable.mul(nativeTokenPrice).div(expandDecimals(1, 18));

  data.stakedSgxLpTrackerAnnualRewardsUsd = stakingData.stakedSgxLpTracker.tokensPerInterval
    .mul(SECONDS_PER_YEAR)
    .mul(gmxPrice)
    .div(expandDecimals(1, 18));
  data.glpAprForEsGmx =
    data.glpSupplyUsd && data.glpSupplyUsd.gt(0)
      ? data.stakedSgxLpTrackerAnnualRewardsUsd.mul(BASIS_POINTS_DIVISOR).div(data.glpSupplyUsd)
      : bigNumberify(0);
  data.feeSgxLpTrackerAnnualRewardsUsd = stakingData.feeSgxLpTracker.tokensPerInterval
    .mul(SECONDS_PER_YEAR)
    .mul(nativeTokenPrice)
    .div(expandDecimals(1, 18));
  data.glpAprForNativeToken =
    data.glpSupplyUsd && data.glpSupplyUsd.gt(0)
      ? data.feeSgxLpTrackerAnnualRewardsUsd.mul(BASIS_POINTS_DIVISOR).div(data.glpSupplyUsd)
      : bigNumberify(0);
  data.glpAprTotal = data.glpAprForNativeToken.add(data.glpAprForEsGmx);

  data.totalSgxLpRewardsUsd = data.stakedSgxLpTrackerRewardsUsd.add(data.feeSgxLpTrackerRewardsUsd);

  data.totalEsGmxRewards = data.stakedGmxTrackerRewards.add(data.stakedSgxLpTrackerRewards);
  data.totalEsGmxRewardsUsd = data.stakedGmxTrackerRewardsUsd.add(data.stakedSgxLpTrackerRewardsUsd);
  ...
  data.totalNativeTokenRewards = data.feeGmxTrackerRewards.add(data.feeSgxLpTrackerRewards);
  data.totalNativeTokenRewardsUsd = data.feeGmxTrackerRewardsUsd.add(data.feeSgxLpTrackerRewardsUsd);
