...
import {
  ...
  AddLiquidity,
  RemoveLiquidity
} from "../generated/SgxLpManager/SgxLpManager"

import {
  Distribute
} from "../generated/FeeSgxRewardDistributor/RewardDistributor"
...
export function handleAddLiquidity(event: AddLiquidity): void {
  _storeSgxLpStat(event.block.timestamp, event.params.sgxlpSupply, event.params.aumInUsdg)
}
...
export function handleDistributeEthToSgx(event: Distribute): void {
  let amount = event.params.amount
  let amountUsd = getTokenAmountUsd(WETH, amount)
  let totalEntity = _getOrCreateSgxStat("total", "total")
  ...
  let entity = _getOrCreateSgxStat(id, "daily")
...
export function handleDistributeEssgxToSgx(event: Distribute): void {
  let amount = event.params.amount
  let amountUsd = getTokenAmountUsd(SGX, amount)

  let totalEntity = _getOrCreateSgxStat("total", "total")
  totalEntity.distributedEssgx += amount
  totalEntity.distributedEssgxCumulative += amount
  totalEntity.distributedEssgxUsd += amountUsd
  totalEntity.distributedEssgxUsdCumulative += amountUsd
  ...
  entity.distributedEssgx += amount
  entity.distributedEssgxCumulative = totalEntity.distributedEthCumulative
  entity.distributedEssgxUsd += amountUsd
  entity.distributedEssgxUsdCumulative = totalEntity.distributedUsdCumulative

  entity.save()
}

function _getOrCreateSgxStat(id: string, period: string): SgxStat {
  let entity = SgxStat.load(id)
  if (entity == null) {
    entity = new SgxStat(id)
    ...
    entity.distributedEssgx = ZERO
    entity.distributedEssgxCumulative = ZERO
    entity.distributedEssgxUsd = ZERO
    entity.distributedEssgxUsdCumulative = ZERO
    entity.period = period
  }
  return entity as SgxStat
}
...
function _getOrCreateSgxLpStat(id: string, period: string): SgxLpStat {
  let entity = SgxLpStat.load(id)
  if (entity == null) {
    entity = new SgxLpStat(id)
    entity.period = period
    entity.sgxlpSupply = ZERO
  ...
  return entity as SgxLpStat
}

export function handleDistributeEthToSgxLp(event: Distribute): void {
  let amount = event.params.amount
  let amountUsd = getTokenAmountUsd(WETH, amount)

  let totalEntity = _getOrCreateSgxLpStat("total", "total")
  totalEntity.distributedEth += amount
  totalEntity.distributedEthCumulative += amount
  totalEntity.distributedUsd += amountUsd
  totalEntity.distributedUsdCumulative += amountUsd
  ...
  let id = _getDayId(event.block.timestamp)
  let entity = _getOrCreateSgxLpStat(id, "daily")
...
export function handleDistributeEssgxToSgxLp(event: Distribute): void {
  let amount = event.params.amount
  let amountUsd = getTokenAmountUsd(SGX, amount)
...
function _storeSgxLpStat(timestamp: BigInt, sgxlpSupply: BigInt, aumInUsdg: BigInt): void {
  let deprecatedId = _getHourId(timestamp)
  let deprecatedEntity = HourlySgxLpStat.load(deprecatedId)

  if (deprecatedEntity == null) {
    deprecatedEntity = new HourlySgxLpStat(deprecatedId)
    deprecatedEntity.sgxlpSupply = ZERO
    deprecatedEntity.aumInUsdg = ZERO
  }

  deprecatedEntity.aumInUsdg = aumInUsdg
  deprecatedEntity.sgxlpSupply = sgxlpSupply
  ...
  totalEntity.sgxlpSupply = sgxlpSupply
  totalEntity.save()

  let entity = _getOrCreateSgxLpStat(id, "daily")
  entity.aumInUsdg = aumInUsdg
  entity.sgxlpSupply = sgxlpSupply
  entity.save()
}
