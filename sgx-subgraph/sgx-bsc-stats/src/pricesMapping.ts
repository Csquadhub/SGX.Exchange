...
export function handleUniswapSgxEthSwap(event: UniswapSwap): void {
  let ethPerSgx = -(event.params.amount0 * BigInt.fromI32(10).pow(18) / event.params.amount1) * BigInt.fromI32(100) / BigInt.fromI32(99)
  let sgxPrice = getTokenAmountUsd(WETH, ethPerSgx)

  let totalId = SGX
  _storeUniswapPrice(totalId, SGX, sgxPrice, "last", event.block.timestamp, event.block.number)

  let id = SGX + ":" + event.block.timestamp.toString()
  _storeUniswapPrice(id, SGX, sgxPrice, "any", event.block.timestamp, event.block.number)
}
