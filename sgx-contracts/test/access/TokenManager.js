...
describe("TokenManager", function () {
  ...
  beforeEach(async () => {
    ...
    //   user0.address, // sgxlpManager
    //   expandDecimals(1000, 18), // maxTokenSupply
    //   10, // marginFeeBasisPoints
    //   100 // maxMarginFeeBasisPoints
    // ])

    // sgxTimelock = await deployContract("SgxTimelock", [
    //   wallet.address,
    //   5 * 24 * 60 * 60,
    //   7 * 24 * 60 * 60,
    //   user0.address,
  ...
  //   await expect(tokenManager.connect(wallet).approve(sgx.address, user2.address, expandDecimals(5, 18), 1))
  //     .to.be.revertedWith("TokenManager: action not signalled")
  ...
  //   await expect(tokenManager.connect(user0).signalSetGov(timelock.address, sgx.address, user1.address))
  //     .to.be.revertedWith("TokenManager: forbidden")

  //   await tokenManager.connect(wallet).signalSetGov(timelock.address, sgx.address, user1.address)
  // })

  // it("signSetGov", async () => {
  //   await expect(tokenManager.connect(user0).signSetGov(timelock.address, sgx.address, user1.address, 1))
  //     .to.be.revertedWith("TokenManager: forbidden")

  //   await expect(tokenManager.connect(signer2).signSetGov(timelock.address, sgx.address, user1.address, 1))
  //     .to.be.revertedWith("TokenManager: action not signalled")
  ...
  //   await tokenManager.connect(signer2).signSetGov(timelock.address, sgx.address, user1.address, 1)

  //   await expect(tokenManager.connect(signer2).signSetGov(timelock.address, sgx.address, user1.address, 1))
  //     .to.be.revertedWith("TokenManager: already signed")

  //   await tokenManager.connect(signer1).signSetGov(timelock.address, sgx.address, user1.address, 1)
  // })

  // it("setGov", async () => {
  //   await sgx.setGov(sgxTimelock.address)

  //   await expect(tokenManager.connect(user0).setGov(sgxTimelock.address, sgx.address, user1.address, 1))
  //     .to.be.revertedWith("TokenManager: forbidden")

  //   await expect(tokenManager.connect(wallet).setGov(sgxTimelock.address, sgx.address, user1.address, 1))
  //     .to.be.revertedWith("TokenManager: action not signalled")

  //   await tokenManager.connect(wallet).signalSetGov(sgxTimelock.address, sgx.address, user1.address)

  //   await expect(tokenManager.connect(wallet).setGov(user2.address, sgx.address, user1.address, 1))
  //     .to.be.revertedWith("TokenManager: action not signalled")

  //   await expect(tokenManager.connect(wallet).setGov(sgxTimelock.address, user0.address, user1.address, 1))
  //     .to.be.revertedWith("TokenManager: action not signalled")

  //   await expect(tokenManager.connect(wallet).setGov(sgxTimelock.address, sgx.address, user2.address, 1))
  //     .to.be.revertedWith("TokenManager: action not signalled")

  //   await expect(tokenManager.connect(wallet).setGov(sgxTimelock.address, sgx.address, user1.address, 1 + 1))
  //     .to.be.revertedWith("TokenManager: action not signalled")

  //   await expect(tokenManager.connect(wallet).setGov(sgxTimelock.address, sgx.address, user1.address, 1))
  //     .to.be.revertedWith("TokenManager: action not authorized")

  //   await tokenManager.connect(signer0).signSetGov(sgxTimelock.address, sgx.address, user1.address, 1)

  //   await expect(tokenManager.connect(wallet).setGov(sgxTimelock.address, sgx.address, user1.address, 1))
  //     .to.be.revertedWith("TokenManager: insufficient authorization")

  //   await expect(sgxTimelock.connect(wallet).signalSetGov(sgx.address, user1.address))
  //     .to.be.revertedWith("SgxTimelock: forbidden")

  //   await tokenManager.connect(signer2).signSetGov(sgxTimelock.address, sgx.address, user1.address, 1)

  //   await expect(sgxTimelock.connect(wallet).setGov(sgx.address, user1.address))
  //     .to.be.revertedWith("SgxTimelock: action not signalled")

  //   await tokenManager.connect(wallet).setGov(sgxTimelock.address, sgx.address, user1.address, 1)
  ...
  //   expect(await sgx.gov()).eq(sgxTimelock.address)
  //   await sgxTimelock.connect(wallet).setGov(sgx.address, user1.address)
  //   expect(await sgx.gov()).eq(user1.address)
  // })
});
