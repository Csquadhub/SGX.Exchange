...
describe("SgxTimelock", function () {
  ...
  beforeEach(async () => {
    ...
    timelock = await deployContract("SgxTimelock", [
      wallet.address,
      5 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
  ...
  it("inits", async () => {
    ...
    await expect(deployContract("SgxTimelock", [
      wallet.address,
      7 * 24 * 60 * 60 + 1,
      7 * 24 + 60 * 60,
      rewardManager.address,
      ...
      mintReceiver.address,
      1000
    ])).to.be.revertedWith("SgxTimelock: invalid _buffer")
    ...
    await expect(deployContract("SgxTimelock", [
      ...
      mintReceiver.address,
      1000
    ])).to.be.revertedWith("SgxTimelock: invalid _longBuffer")
  })
  ...
  it("setTokenConfig", async () => {
    ...
    await expect(timelock.connect(wallet).setTokenConfig(
      ...
      0,
      0
    )).to.be.revertedWith("SgxTimelock: token not yet whitelisted")
  ...
  it("setBuffer", async () => {
    const timelock0 = await deployContract("SgxTimelock", [
      user1.address,
      3 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
      rewardManager.address,
    ...
    await expect(timelock0.connect(user1).setBuffer(3 * 24 * 60 * 60 - 10))
      .to.be.revertedWith("SgxTimelock: buffer cannot be decreased")
  ...
  it("setMaxLeverage", async () => {
    ...
    await expect(timelock.connect(wallet).setMaxLeverage(vault.address, 49 * 10000))
      .to.be.revertedWith("SgxTimelock: invalid _maxLeverage")
  ...
  it("approve", async () => {
    ...
    await expect(timelock.connect(wallet).cancelAction(action0))
      .to.be.revertedWith("SgxTimelock: invalid _action")
  ...
  it("processMint", async () => {
    ...
    await sgx.setGov(timelock.address)

    await expect(timelock.connect(user0).processMint(sgx.address, user1.address, expandDecimals(100, 18)))
      .to.be.revertedWith("SgxTimelock: forbidden")

    await expect(timelock.connect(wallet).processMint(sgx.address, user1.address, expandDecimals(100, 18)))
      .to.be.revertedWith("SgxTimelock: action not signalled")

    await expect(timelock.connect(user0).signalMint(sgx.address, user1.address, expandDecimals(100, 18)))
      .to.be.revertedWith("SgxTimelock: forbidden")

    await timelock.connect(wallet).signalMint(sgx.address, user1.address, expandDecimals(100, 18))
    ...
    await expect(timelock.connect(wallet).processMint(sgx.address, user2.address, expandDecimals(100, 18)))
      .to.be.revertedWith("SgxTimelock: action not signalled")

    await expect(timelock.connect(wallet).processMint(sgx.address, user1.address, expandDecimals(101, 18)))
      .to.be.revertedWith("SgxTimelock: action not signalled")

    expect(await sgx.balanceOf(timelock.address)).eq(0)
    expect(await sgx.balanceOf(user1.address)).eq(0)

    await timelock.connect(wallet).processMint(sgx.address, user1.address, expandDecimals(100, 18))
    ...
    const action1 = ethers.utils.solidityKeccak256(["string", "address", "address", "uint256"], ["mint", sgx.address, user1.address, expandDecimals(100, 18)])
  ...
  it("withdrawToken", async () => {
    ...
    await expect(timelock.connect(user0).withdrawToken(sgx.address, bnb.address, user0.address, 100))
      .to.be.revertedWith("SgxTimelock: forbidden")

    await expect(timelock.connect(wallet).withdrawToken(sgx.address, bnb.address, user0.address, 100))
      .to.be.revertedWith("SgxTimelock: action not signalled")

    await expect(timelock.connect(user0).signalWithdrawToken(sgx.address, bnb.address, user0.address, 100))
      .to.be.revertedWith("SgxTimelock: forbidden")

    await timelock.connect(wallet).signalWithdrawToken(sgx.address, bnb.address, user0.address, 100)
    ...
    await expect(timelock.connect(wallet).withdrawToken(sgx.address, dai.address, user0.address, 100))
      .to.be.revertedWith("SgxTimelock: action not signalled")

    await expect(timelock.connect(wallet).withdrawToken(sgx.address, bnb.address, user1.address, 100))
      .to.be.revertedWith("SgxTimelock: action not signalled")

    await expect(timelock.connect(wallet).withdrawToken(sgx.address, bnb.address, user0.address, 101))
      .to.be.revertedWith("SgxTimelock: action not signalled")

    await expect(timelock.connect(wallet).withdrawToken(sgx.address, bnb.address, user0.address, 100))
      .to.be.revertedWith("ERC20: transfer amount exceeds balance")

    await bnb.mint(sgx.address, 100)
    expect(await bnb.balanceOf(user0.address)).eq(0)
    await timelock.connect(wallet).withdrawToken(sgx.address, bnb.address, user0.address, 100)
  ...
  it("addExcludedToken", async () => {
    const sgx = await deployContract("SGX", [])
    await expect(timelock.connect(user0).addExcludedToken(sgx.address))
      .to.be.revertedWith("SgxTimelock: forbidden")

    expect(await timelock.excludedTokens(sgx.address)).eq(false)
    await timelock.connect(wallet).addExcludedToken(sgx.address)
    expect(await timelock.excludedTokens(sgx.address)).eq(true)
  })
  ...
  it("setInPrivateTransferMode", async () => {
    ...
    await expect(timelock.connect(user0).setInPrivateTransferMode(sgx.address, true))
      .to.be.revertedWith("SgxTimelock: forbidden")

    await expect(timelock.connect(wallet).setInPrivateTransferMode(sgx.address, true))
      .to.be.revertedWith("BaseToken: forbidden")

    await sgx.setGov(timelock.address)

    expect(await sgx.inPrivateTransferMode()).eq(false)
    await timelock.connect(wallet).setInPrivateTransferMode(sgx.address, true)
    expect(await sgx.inPrivateTransferMode()).eq(true)

    await timelock.connect(wallet).setInPrivateTransferMode(sgx.address, false)
    expect(await sgx.inPrivateTransferMode()).eq(false)

    await timelock.connect(wallet).setInPrivateTransferMode(sgx.address, true)
    expect(await sgx.inPrivateTransferMode()).eq(true)

    await expect(sgx.connect(user0).transfer(user1.address, 100))
      .to.be.revertedWith("BaseToken: msg.sender not whitelisted")

    await timelock.addExcludedToken(sgx.address)
    await expect(timelock.connect(wallet).setInPrivateTransferMode(sgx.address, true))
      .to.be.revertedWith("SgxTimelock: invalid _inPrivateTransferMode")

    await timelock.connect(wallet).setInPrivateTransferMode(sgx.address, false)
    expect(await sgx.inPrivateTransferMode()).eq(false)

    await sgx.connect(user0).transfer(user1.address, 100)
  })
  ...
  it("setExternalAdmin", async () => {
    ...
    await expect(timelock.connect(wallet).setExternalAdmin(timelock.address, user3.address))
      .to.be.revertedWith("SgxTimelock: invalid _target")
  })
