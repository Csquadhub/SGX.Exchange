...
describe("Vault.swap", function () {
  ...
  it("swap", async () => {
    ...
    expect(await sgxlpManager.getAumInSgusd(false)).eq(0)
    ...
    expect(await sgxlpManager.getAumInSgusd(false)).eq(expandDecimals(59820, 18)) // 60,000 * 99.7%
    ...
    expect(await sgxlpManager.getAumInSgusd(false)).eq(expandDecimals(119640, 18)) // 59,820 + (60,000 * 99.7%)

    expect(await sgusd.balanceOf(user0.address)).eq(expandDecimals(120000, 18).sub(expandDecimals(360, 18))) // 120,000 * 0.3% => 360

    expect(await vault.feeReserves(bnb.address)).eq("600000000000000000") // 200 * 0.3% => 0.6
    expect(await vault.sgusdAmounts(bnb.address)).eq(expandDecimals(200 * 300, 18).sub(expandDecimals(180, 18))) // 60,000 * 0.3% => 180
    ...
    expect(await vault.sgusdAmounts(btc.address)).eq(expandDecimals(200 * 300, 18).sub(expandDecimals(180, 18)))
    ...
    expect(await sgxlpManager.getAumInSgusd(false)).eq(expandDecimals(139580, 18)) // 59,820 / 300 * 400 + 59820
    ...
    expect(await sgxlpManager.getAumInSgusd(false)).eq(expandDecimals(159520, 18)) // 59,820 / 300 * 400 + 59820 / 60000 * 80000
    ...
    expect(await sgxlpManager.getAumInSgusd(false)).eq(expandDecimals(167520, 18)) // 159520 + (100 * 400) - 32000

    expect(await btc.balanceOf(user1.address)).eq(0)
    expect(await btc.balanceOf(user2.address)).eq(expandDecimals(4, 7).sub("120000")) // 0.8 - 0.0012

    expect(await vault.feeReserves(bnb.address)).eq("600000000000000000") // 200 * 0.3% => 0.6
    expect(await vault.sgusdAmounts(bnb.address)).eq(expandDecimals(100 * 400, 18).add(expandDecimals(200 * 300, 18)).sub(expandDecimals(180, 18)))
    ...
    expect(await vault.sgusdAmounts(btc.address)).eq(expandDecimals(200 * 300, 18).sub(expandDecimals(180, 18)).sub(expandDecimals(100 * 400, 18)))
    ...
    await sgusd.connect(user0).transfer(vault.address, expandDecimals(50000, 18))
    await vault.sellSGUSD(bnb.address, user3.address)
    expect(await bnb.balanceOf(user0.address)).eq(0)
    expect(await bnb.balanceOf(user3.address)).eq("99700000000000000000") // 99.7, 50000 / 500 * 99.7%

    await sgusd.connect(user0).transfer(vault.address, expandDecimals(50000, 18))
    await vault.sellSGUSD(btc.address, user3.address)

    await sgusd.connect(user0).transfer(vault.address, expandDecimals(10000, 18))
    await expect(vault.sellSGUSD(btc.address, user3.address))
      .to.be.revertedWith("Vault: poolAmount exceeded")
  })

  it("caps max SGUSD amount", async () => {
    await bnbPriceFeed.setLatestAnswer(toChainlinkPrice(600))
    ...
    await vault.connect(user0).buySGUSD(eth.address, user1.address)

    await bnb.mint(user0.address, expandDecimals(1, 18))
    await bnb.connect(user0).transfer(vault.address, expandDecimals(1, 18))

    await expect(vault.connect(user0).buySGUSD(bnb.address, user0.address))
      .to.be.revertedWith("Vault: max SGUSD exceeded")
  ...
  it("does not cap max SGUSD debt", async () => {
    await bnbPriceFeed.setLatestAnswer(toChainlinkPrice(600))
    await vault.setTokenConfig(...getBnbConfig(bnb, bnbPriceFeed))
