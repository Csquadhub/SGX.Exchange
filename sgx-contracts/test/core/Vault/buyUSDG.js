...
describe("Vault.buySGUSD", function () {
  ...
  it("buySGUSD", async () => {
    await expect(vault.buySGUSD(bnb.address, wallet.address))
      .to.be.revertedWith("Vault: _token not whitelisted")

    await expect(vault.connect(user0).buySGUSD(bnb.address, user1.address))
      .to.be.revertedWith("Vault: _token not whitelisted")
    ...
    const tx = await vault.connect(user0).buySGUSD(bnb.address, user1.address, { gasPrice: "10000000000" })
    await reportGasUsed(provider, tx, "buySGUSD gas used")

    expect(await sgusd.balanceOf(user0.address)).eq(0)
    expect(await sgusd.balanceOf(user1.address)).eq(29700)
  ...
  it("buySGUSD allows gov to mint", async () => {
    ...
    expect(await sgusd.balanceOf(wallet.address)).eq(0)
    expect(await vault.feeReserves(bnb.address)).eq(0)
    expect(await vault.sgusdAmounts(bnb.address)).eq(0)
    expect(await vault.poolAmounts(bnb.address)).eq(0)

    await expect(vault.connect(user0).buySGUSD(bnb.address, wallet.address))
      .to.be.revertedWith("Vault: forbidden")

    await vault.setManager(user0.address, true)
    await vault.connect(user0).buySGUSD(bnb.address, wallet.address)

    expect(await sgusd.balanceOf(wallet.address)).eq(29700)
    expect(await vault.feeReserves(bnb.address)).eq(1)
    expect(await vault.sgusdAmounts(bnb.address)).eq(29700)
    expect(await vault.poolAmounts(bnb.address)).eq(100 - 1)
  ...
  it("buySGUSD uses min price", async () => {
    await expect(vault.connect(user0).buySGUSD(bnb.address, user1.address))
      .to.be.revertedWith("Vault: _token not whitelisted")
    ...
    await bnb.mint(user0.address, 100)
    await bnb.connect(user0).transfer(vault.address, 100)
    await vault.connect(user0).buySGUSD(bnb.address, user1.address)
    expect(await sgusd.balanceOf(user0.address)).eq(0)
    expect(await sgusd.balanceOf(user1.address)).eq(19800)
    expect(await vault.feeReserves(bnb.address)).eq(1)
    expect(await vault.sgusdAmounts(bnb.address)).eq(19800)
    expect(await vault.poolAmounts(bnb.address)).eq(100 - 1)

    await validateVaultBalance(expect, vault, bnb)
  })

  it("buySGUSD updates fees", async () => {
    await expect(vault.connect(user0).buySGUSD(bnb.address, user1.address))
      .to.be.revertedWith("Vault: _token not whitelisted")
    ...
    await vault.connect(user0).buySGUSD(bnb.address, user1.address)
    expect(await sgusd.balanceOf(user0.address)).eq(0)
    expect(await sgusd.balanceOf(user1.address)).eq(9970 * 300)
    expect(await vault.feeReserves(bnb.address)).eq(30)
    expect(await vault.sgusdAmounts(bnb.address)).eq(9970 * 300)
    expect(await vault.poolAmounts(bnb.address)).eq(10000 - 30)

    await validateVaultBalance(expect, vault, bnb)
  })

  it("buySGUSD uses mintBurnFeeBasisPoints", async () => {
    ...
    await vault.connect(user0).buySGUSD(dai.address, user1.address)
    expect(await sgusd.balanceOf(user0.address)).eq(0)
    expect(await sgusd.balanceOf(user1.address)).eq(expandDecimals(10000 - 4, 18))
    expect(await vault.feeReserves(dai.address)).eq(expandDecimals(4, 18))
    expect(await vault.sgusdAmounts(dai.address)).eq(expandDecimals(10000 - 4, 18))
    expect(await vault.poolAmounts(dai.address)).eq(expandDecimals(10000 - 4, 18))
  })

  it("buySGUSD adjusts for decimals", async () => {
    await btcPriceFeed.setLatestAnswer(toChainlinkPrice(60000))
    await vault.setTokenConfig(...getBtcConfig(btc, btcPriceFeed))

    await expect(vault.connect(user0).buySGUSD(btc.address, user1.address))
      .to.be.revertedWith("Vault: invalid tokenAmount")
    ...
    await vault.connect(user0).buySGUSD(btc.address, user1.address)
    expect(await sgusd.balanceOf(user0.address)).eq(0)
    expect(await vault.feeReserves(btc.address)).eq(300000)
    expect(await sgusd.balanceOf(user1.address)).eq(expandDecimals(60000, 18).sub(expandDecimals(180, 18))) // 0.3% of 60,000 => 180
    expect(await vault.sgusdAmounts(btc.address)).eq(expandDecimals(60000, 18).sub(expandDecimals(180, 18)))
