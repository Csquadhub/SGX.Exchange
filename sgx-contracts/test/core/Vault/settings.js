...
describe("Vault.settings", function () {
  ...
  beforeEach(async () => {
    ...
    const contracts = await initVault(vault, router, sgusd, vaultPriceFeed)
  ...
  it("setTokenConfig", async () => {
    ...
    expect(await vault.maxSgusdAmounts(bnb.address)).eq(0)
    ...
    expect(await vault.maxSgusdAmounts(dai.address)).eq(1000)
    ...
    expect(await vault.maxSgusdAmounts(dai.address)).eq(500)
  ...
  it("clearTokenConfig", async () => {
    ...
    expect(await vault.minProfitBasisPoints(bnb.address)).eq(75)
    expect(await vault.maxSgusdAmounts(bnb.address)).eq(500)
    expect(await vault.stableTokens(bnb.address)).eq(true)
    expect(await vault.shortableTokens(bnb.address)).eq(true)
  ...
  it("setSgusdAmount", async () => {
    ...
    await expect(vault.connect(user0).setSgusdAmount(bnb.address, 50000))
      .to.be.revertedWith("Vault: forbidden")

    await vault.setGov(user0.address)

    expect(await vault.sgusdAmounts(bnb.address)).eq(29700)
    await vault.connect(user0).setSgusdAmount(bnb.address, 50000)
    expect(await vault.sgusdAmounts(bnb.address)).eq(50000)

    await vault.connect(user0).setSgusdAmount(bnb.address, 10000)
    expect(await vault.sgusdAmounts(bnb.address)).eq(10000)
  })
