...
describe("Vault.increaseLongPosition", function () {
  ...
  it("increasePosition long", async () => {
    ...
    expect(await sgxlpManager.getAumInSgusd(true)).eq("48029860000000000000") // 48.02986
    expect(await sgxlpManager.getAumInSgusd(false)).eq("46858400000000000000") // 46.8584

    expect(await vault.feeReserves(btc.address)).eq(353) // (117500 - 1) * 0.3% => 353
    expect(await vault.sgusdAmounts(btc.address)).eq("46858400000000000000") // (117500 - 1 - 353) * 40000
    ...
    expect(await sgxlpManager.getAumInSgusd(true)).eq("96059720000000000000") // 96.05972
    expect(await sgxlpManager.getAumInSgusd(false)).eq("93716800000000000000") // 93.7168

    expect(await vault.feeReserves(btc.address)).eq(353 * 2) // (117500 - 1) * 0.3% * 2
    expect(await vault.sgusdAmounts(btc.address)).eq("93716800000000000000") // (117500 - 1 - 353) * 40000 * 2
    ...
    expect(await sgxlpManager.getAumInSgusd(true)).eq("95109980000000000000") // 95.10998
    expect(await sgxlpManager.getAumInSgusd(false)).eq("93718200000000000000") // 93.7182
  ...
  it("increasePosition long aum", async () => {
    ...
    expect(await sgxlpManager.getAumInSgusd(true)).eq(expandDecimals(99700, 18))

    expect(await vault.feeReserves(btc.address)).eq("300000") // 0.003 BTC
    expect(await vault.sgusdAmounts(btc.address)).eq(expandDecimals(99700, 18))
    ...
    expect(await vault.getRedemptionCollateralUsd(btc.address)).eq(toUsd(99700))
    expect(await sgxlpManager.getAumInSgusd(true)).eq(expandDecimals(99700, 18))
    expect(await sgxlpManager.getAumInSgusd(false)).eq(expandDecimals(99700, 18))
    ...
    expect(await sgxlpManager.getAumInSgusd(true)).eq(expandDecimals(134510, 18)) // 30080 + (1.4962-0.8)*150000
    expect(await sgxlpManager.getAumInSgusd(false)).eq(expandDecimals(134510, 18)) // 30080 + (1.4962-0.8)*150000
    ...
    expect(delta[1]).eq(toUsd(40000))
    expect(await sgxlpManager.getAumInSgusd(true)).eq(expandDecimals(82295, 18)) // 30080 + (1.4962-0.8)*75000
    expect(await sgxlpManager.getAumInSgusd(false)).eq(expandDecimals(64890, 18)) // 30080 + (1.4962-0.8)*50000
    ...
    expect(await sgxlpManager.getAumInSgusd(true)).eq("102295000500000000000000") // 102295.0005
    expect(await sgxlpManager.getAumInSgusd(false)).eq("68196667000000000000000") // 68196.667
