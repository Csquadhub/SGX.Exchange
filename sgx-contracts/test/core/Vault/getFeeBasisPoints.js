...
describe("Vault.getFeeBasisPoints", function () {
  ...
  it("getFeeBasisPoints", async () => {
    ...
    expect(await vault.getTargetSgusdAmount(bnb.address)).eq(0)
    ...
    expect(await vault.getTargetSgusdAmount(bnb.address)).eq(29700)

    // sgusdAmount(bnb) is 29700, targetAmount(bnb) is 29700
    ...
    expect(await vault.getTargetSgusdAmount(bnb.address)).eq(14850)
    expect(await vault.getTargetSgusdAmount(dai.address)).eq(14850)

    // sgusdAmount(bnb) is 29700, targetAmount(bnb) is 14850
    ...
    await vault.connect(user0).buySGUSD(dai.address, wallet.address)

    expect(await vault.getTargetSgusdAmount(bnb.address)).eq(24850)
    expect(await vault.getTargetSgusdAmount(dai.address)).eq(24850)

    const bnbConfig = getBnbConfig(bnb, bnbPriceFeed)
    bnbConfig[2] = 30000
    await vault.setTokenConfig(...bnbConfig)

    expect(await vault.getTargetSgusdAmount(bnb.address)).eq(37275)
    expect(await vault.getTargetSgusdAmount(dai.address)).eq(12425)

    expect(await vault.sgusdAmounts(bnb.address)).eq(29700)

    // sgusdAmount(bnb) is 29700, targetAmount(bnb) is 37270
    ...
    expect(await vault.sgusdAmounts(bnb.address)).eq(89100)
    expect(await vault.getTargetSgusdAmount(bnb.address)).eq(36366)
    expect(await vault.getTargetSgusdAmount(dai.address)).eq(72733)

    // sgusdAmount(bnb) is 88800, targetAmount(bnb) is 36266
    // incrementing bnb has an increased fee, while reducing bnb has a decreased fee
    expect(await vault.getFeeBasisPoints(bnb.address, 1000, 100, 50, true)).eq(150)
    expect(await vault.getFeeBasisPoints(bnb.address, 5000, 100, 50, true)).eq(150)
    expect(await vault.getFeeBasisPoints(bnb.address, 10000, 100, 50, true)).eq(150)
