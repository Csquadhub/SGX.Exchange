...
describe("Router", function () {
  ...
  it("swap, buy SGUSD", async () => {
    ...
    await expect(router.connect(user0).swap([dai.address, sgusd.address], expandDecimals(200, 18), expandDecimals(201, 18), user0.address))
    ...
    const tx = await router.connect(user0).swap([dai.address, sgusd.address], expandDecimals(200, 18), expandDecimals(199, 18), user0.address)
    await reportGasUsed(provider, tx, "buySGUSD gas used")
    expect(await dai.balanceOf(user0.address)).eq(0)
    expect(await sgusd.balanceOf(user0.address)).eq("199400000000000000000") // 199.4
  })

  it("swap, sell SGUSD", async () => {
    await dai.mint(user0.address, expandDecimals(200, 18))
    await dai.connect(user0).approve(router.address, expandDecimals(200, 18))
    ...
    await sgusd.connect(user0).approve(router.address, expandDecimals(100, 18))
    await expect(router.connect(user0).swap([sgusd.address, dai.address], expandDecimals(100, 18), expandDecimals(100, 18), user0.address))
      .to.be.revertedWith("Router: insufficient amountOut")

    await router.connect(user0).swap([sgusd.address, dai.address], expandDecimals(100, 18), expandDecimals(99, 18), user0.address)
    expect(await dai.balanceOf(user0.address)).eq("99700000000000000000") // 99.7
    expect(await sgusd.balanceOf(user0.address)).eq("99400000000000000000") // 99.4
  })

  it("swap, path.length == 2", async () => {
    await btc.mint(user0.address, expandDecimals(1, 8))
    await btc.connect(user0).approve(router.address, expandDecimals(1, 8))
    await expect(router.connect(user0).swap([btc.address, sgusd.address], expandDecimals(1, 8), expandDecimals(60000, 18), user0.address))
      .to.be.revertedWith("Router: insufficient amountOut")
    await router.connect(user0).swap([btc.address, sgusd.address], expandDecimals(1, 8), expandDecimals(59000, 18), user0.address)
  ...
  it("swap, path.length == 3", async () => {
    ...
    await dai.mint(user0.address, expandDecimals(30000, 18))
    await dai.connect(user0).approve(router.address, expandDecimals(30000, 18))
    await router.connect(user0).swap([dai.address, sgusd.address], expandDecimals(30000, 18), expandDecimals(29000, 18), user0.address)

    await sgusd.connect(user0).approve(router.address, expandDecimals(20000, 18))

    expect(await dai.balanceOf(user0.address)).eq(0)
    expect(await sgusd.balanceOf(user0.address)).eq(expandDecimals(89730, 18))
    await expect(router.connect(user0).swap([sgusd.address, dai.address, sgusd.address], expandDecimals(20000, 18), expandDecimals(20000, 18), user0.address))
      .to.be.revertedWith("Router: insufficient amountOut")

    await router.connect(user0).swap([sgusd.address, dai.address, sgusd.address], expandDecimals(20000, 18), expandDecimals(19000, 18), user0.address)
    expect(await dai.balanceOf(user0.address)).eq(0)
    expect(await sgusd.balanceOf(user0.address)).eq("89610180000000000000000") // 89610.18

    await sgusd.connect(user0).approve(router.address, expandDecimals(40000, 18))
    await expect(router.connect(user0).swap([sgusd.address, dai.address, btc.address], expandDecimals(30000, 18), expandDecimals(39000, 18), user0.address))
    ...
    await expect(router.connect(user0).swap([sgusd.address, dai.address, btc.address], expandDecimals(20000, 18), "34000000", user0.address))
      .to.be.revertedWith("Router: insufficient amountOut")

    const tx = await router.connect(user0).swap([sgusd.address, dai.address, btc.address], expandDecimals(20000, 18), "33000000", user0.address)
    await reportGasUsed(provider, tx, "swap gas used")
    expect(await sgusd.balanceOf(user0.address)).eq("69610180000000000000000") // 69610.18
  ...
  it("decreasePositionAndSwapETH", async () => {
    ...
    await router.connect(user0).swap([bnb.address, sgusd.address], expandDecimals(10, 18), expandDecimals(2900, 18), user0.address)
