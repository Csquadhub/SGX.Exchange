...
describe("OrderBook, swap orders", function () {
    ...
    beforeEach(async () => {
        ...
        // it's impossible to just mint sgusd (?)
        await router.connect(user0).swap(
            [dai.address, sgusd.address],
            expandDecimals(10000, 18),
            expandDecimals(9900, 18),
            user0.address
        );
        await sgusd.connect(user0).approve(router.address, expandDecimals(9900, 18));
    ...
    */
    ...
    it("executeSwapOrder, triggerAboveThreshold == true, SGUSD -> BTC", async () => {
        const triggerRatio = getTriggerRatio(toUsd(1), toUsd(62000));
        const amountIn = expandDecimals(1000, 18);
        const path = [sgusd.address, btc.address];
        const value = defaults.executionFee;
    ...
    it("executeSwapOrder, triggerAboveThreshold == true, SGUSD -> DAI -> BTC", async () => {
        const triggerRatio = getTriggerRatio(toUsd(1), toUsd(62000));
        const amountIn = expandDecimals(1000, 18);
        const path = [sgusd.address, dai.address, btc.address];
        const value = defaults.executionFee;
    ...
    it("executeSwapOrder, triggerAboveThreshold == true, SGUSD -> BNB -> BTC", async () => {
        const triggerRatio = getTriggerRatio(toUsd(1), toUsd(62000));
        const amountIn = expandDecimals(1000, 18);
        const path = [sgusd.address, bnb.address, btc.address];
        const value = defaults.executionFee;
    ...
    it("executeSwapOrder, triggerAboveThreshold == true, BTC -> SGUSD", async () => {
        const triggerRatio = getTriggerRatio(toUsd(62000), toUsd(1));
        const amountIn = expandDecimals(1, 6); // 0.01 BTC
        const path = [btc.address, sgusd.address];
        const value = defaults.executionFee;
        ...
        const executorBalanceBefore = await executor.getBalance();
        const userSgusdBalanceBefore = await sgusd.balanceOf(defaults.user.address);
        ...
        const userSgusdBalanceAfter = await sgusd.balanceOf(defaults.user.address);
        expect(userSgusdBalanceAfter.gt(userSgusdBalanceBefore.add(minOut)), 'userSgusdBalanceAfter').to.be.true;
