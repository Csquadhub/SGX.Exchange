...
contract OrderBook is ReentrancyGuard, IOrderBook {
    ...
    uint256 public constant SGUSD_PRECISION = 1e18;
    ...
    function initialize(
        ...
        emit Initialize(_router, _vault, _weth, _sgusd, _minExecutionFee, _minPurchaseTokenAmountUsd);
    }
    ...
    function getSgusdMinPrice(address _otherToken) public view returns (uint256) {
        // SGUSD_PRECISION is the same as 1 SGUSD
        uint256 redemptionAmount = IVault(vault).getRedemptionAmount(_otherToken, SGUSD_PRECISION);
    ...
    function validateSwapOrderPriceWithTriggerAboveThreshold(
        ...
        // 1. SGUSD doesn't have a price feed so we need to calculate it based on redepmtion amount of a specific token
        // That's why SGUSD price in USD can vary depending on the redepmtion token
        // 2. In complex scenarios with path=[SGUSD, BNB, BTC] we need to know how much BNB we'll get for provided SGUSD
        // to know how much BTC will be received
        // That's why in such scenario BNB should be used to determine price of SGUSD
        if (tokenA == sgusd) {
            // with both _path.length == 2 or 3 we need sgusd price against _path[1]
            tokenAPrice = getSgusdMinPrice(_path[1]);
        } else {
    ...
    function _vaultSwap(address _tokenIn, address _tokenOut, uint256 _minOut, address _receiver) private returns (uint256) {
        ...
        if (_tokenOut == sgusd) { // buySGUSD
            amountOut = IVault(vault).buySGUSD(_tokenIn, _receiver);
        } else if (_tokenIn == sgusd) { // sellSGUSD
            amountOut = IVault(vault).sellSGUSD(_tokenOut, _receiver);
        } else { // swap
