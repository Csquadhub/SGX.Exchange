...
contract Vault is ReentrancyGuard, IVault {
    ...
    // sgusdAmounts tracks the amount of SGUSD debt for each whitelisted token
    mapping (address => uint256) public override sgusdAmounts;

    // maxSgusdAmounts allows setting a max amount of SGUSD debt for a token
    mapping (address => uint256) public override maxSgusdAmounts;
    ...
    // this value is used to calculate the redemption values for selling of SGUSD
    ...
    event BuySGUSD(address account, address token, uint256 tokenAmount, uint256 sgusdAmount, uint256 feeBasisPoints);
    event SellSGUSD(address account, address token, uint256 sgusdAmount, uint256 tokenAmount, uint256 feeBasisPoints);
    ...
    event IncreaseSgusdAmount(address token, uint256 amount);
    event DecreaseSgusdAmount(address token, uint256 amount);
    ...
    function setTokenConfig(
        ...
        maxSgusdAmounts[_token] = _maxSgusdAmount;
    ...
    function setSgusdAmount(address _token, uint256 _amount) external override {
        _onlyGov();

        uint256 sgusdAmount = sgusdAmounts[_token];
        if (_amount > sgusdAmount) {
            _increaseSgusdAmount(_token, _amount.sub(sgusdAmount));
            return;
        }

        _decreaseSgusdAmount(_token, sgusdAmount.sub(_amount));
    }
    ...
    // deposit into the pool without minting SGUSD tokens
    ...
    function buySGUSD(address _token, address _receiver) external override nonReentrant returns (uint256) {
        _validateManager();
        ...
        uint256 sgusdAmount = tokenAmount.mul(price).div(PRICE_PRECISION);
        sgusdAmount = adjustForDecimals(sgusdAmount, _token, sgusd);
        _validate(sgusdAmount > 0, 18);

        uint256 feeBasisPoints = vaultUtils.getBuySgusdFeeBasisPoints(_token, sgusdAmount);
        uint256 amountAfterFees = _collectSwapFees(_token, tokenAmount, feeBasisPoints);
        uint256 mintAmount = amountAfterFees.mul(price).div(PRICE_PRECISION);
        mintAmount = adjustForDecimals(mintAmount, _token, sgusd);

        _increaseSgusdAmount(_token, mintAmount);
        _increasePoolAmount(_token, amountAfterFees);

        ISGUSD(sgusd).mint(_receiver, mintAmount);

        emit BuySGUSD(_receiver, _token, tokenAmount, mintAmount, feeBasisPoints);

        useSwapPricing = false;
        return mintAmount;
    }

    function sellSGUSD(address _token, address _receiver) external override nonReentrant returns (uint256) {
        _validateManager();
        _validate(whitelistedTokens[_token], 19);
        useSwapPricing = true;

        uint256 sgusdAmount = _transferIn(sgusd);
        _validate(sgusdAmount > 0, 20);

        updateCumulativeFundingRate(_token, _token);

        uint256 redemptionAmount = getRedemptionAmount(_token, sgusdAmount);
        _validate(redemptionAmount > 0, 21);

        _decreaseSgusdAmount(_token, sgusdAmount);
        _decreasePoolAmount(_token, redemptionAmount);

        ISGUSD(sgusd).burn(address(this), sgusdAmount);

        // the _transferIn call increased the value of tokenBalances[sgusd]
        // usually decreases in token balances are synced by calling _transferOut
        // however, for sgusd, the tokens are burnt, so _updateTokenBalance should
        // be manually called to record the decrease in tokens
        _updateTokenBalance(sgusd);

        uint256 feeBasisPoints = vaultUtils.getSellSgusdFeeBasisPoints(_token, sgusdAmount);
        uint256 amountOut = _collectSwapFees(_token, redemptionAmount, feeBasisPoints);
        _validate(amountOut > 0, 22);

        _transferOut(_token, amountOut, _receiver);

        emit SellSGUSD(_receiver, _token, sgusdAmount, amountOut, feeBasisPoints);
    ...
    function swap(address _tokenIn, address _tokenOut, address _receiver) external override nonReentrant returns (uint256) {
        ...
        // adjust sgusdAmounts by the same sgusdAmount as debt is shifted between the assets
        uint256 sgusdAmount = amountIn.mul(priceIn).div(PRICE_PRECISION);
        sgusdAmount = adjustForDecimals(sgusdAmount, _tokenIn, sgusd);

        uint256 feeBasisPoints = vaultUtils.getSwapFeeBasisPoints(_tokenIn, _tokenOut, sgusdAmount);
        uint256 amountOutAfterFees = _collectSwapFees(_tokenOut, amountOut, feeBasisPoints);

        _increaseSgusdAmount(_tokenIn, sgusdAmount);
        _decreaseSgusdAmount(_tokenOut, sgusdAmount);
    ...
    function getRedemptionAmount(address _token, uint256 _sgusdAmount) public override view returns (uint256) {
        uint256 price = getMaxPrice(_token);
        uint256 redemptionAmount = _sgusdAmount.mul(PRICE_PRECISION).div(price);
        return adjustForDecimals(redemptionAmount, sgusd, _token);
    }
    ...
    function adjustForDecimals(uint256 _amount, address _tokenDiv, address _tokenMul) public view returns (uint256) {
        uint256 decimalsDiv = _tokenDiv == sgusd ? SGUSD_DECIMALS : tokenDecimals[_tokenDiv];
        uint256 decimalsMul = _tokenMul == sgusd ? SGUSD_DECIMALS : tokenDecimals[_tokenMul];
    ...
    function getFeeBasisPoints(address _token, uint256 _sgusdDelta, uint256 _feeBasisPoints, uint256 _taxBasisPoints, bool _increment) public override view returns (uint256) {
        return vaultUtils.getFeeBasisPoints(_token, _sgusdDelta, _feeBasisPoints, _taxBasisPoints, _increment);
    }

    function getTargetSgusdAmount(address _token) public override view returns (uint256) {
        uint256 supply = IERC20(sgusd).totalSupply();
    ...
    function _increaseSgusdAmount(address _token, uint256 _amount) private {
        sgusdAmounts[_token] = sgusdAmounts[_token].add(_amount);
        uint256 maxSgusdAmount = maxSgusdAmounts[_token];
        if (maxSgusdAmount != 0) {
            _validate(sgusdAmounts[_token] <= maxSgusdAmount, 51);
        }
        emit IncreaseSgusdAmount(_token, _amount);
    }

    function _decreaseSgusdAmount(address _token, uint256 _amount) private {
        uint256 value = sgusdAmounts[_token];
        // since SGUSD can be minted using multiple assets
        // it is possible for the SGUSD debt for a single asset to be less than zero
        // the SGUSD debt is capped to zero for this case
        if (value <= _amount) {
            sgusdAmounts[_token] = 0;
            emit DecreaseSgusdAmount(_token, value);
            return;
        }
        sgusdAmounts[_token] = value.sub(_amount);
        emit DecreaseSgusdAmount(_token, _amount);
    }
