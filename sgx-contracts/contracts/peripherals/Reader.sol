...
contract Reader is Governable {
    ...
    function getMaxAmountIn(IVault _vault, address _tokenIn, address _tokenOut) public view returns (uint256) {
        ...
        uint256 maxSgusdAmount = _vault.maxSgusdAmounts(_tokenIn);

        if (maxSgusdAmount != 0) {
            if (maxSgusdAmount < _vault.sgusdAmounts(_tokenIn)) {
                return 0;
            }

            uint256 maxAmountIn = maxSgusdAmount.sub(_vault.sgusdAmounts(_tokenIn));
            maxAmountIn = maxAmountIn.mul(10 ** tokenInDecimals).div(10 ** SGUSD_DECIMALS);
    ...
    function getAmountOut(IVault _vault, address _tokenIn, address _tokenOut, uint256 _amountIn) public view returns (uint256, uint256) {
        ...
        {
            uint256 sgusdAmount = _amountIn.mul(priceIn).div(PRICE_PRECISION);
            sgusdAmount = sgusdAmount.mul(10 ** SGUSD_DECIMALS).div(10 ** tokenInDecimals);
            ...
            uint256 feesBasisPoints0 = _vault.getFeeBasisPoints(_tokenIn, sgusdAmount, baseBps, taxBps, true);
            uint256 feesBasisPoints1 = _vault.getFeeBasisPoints(_tokenOut, sgusdAmount, baseBps, taxBps, false);
    ...
    function getVaultTokenInfo(address _vault, address _weth, uint256 _sgusdAmount, address[] memory _tokens) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 2] = vault.sgusdAmounts(token);
            amounts[i * propsLength + 3] = vault.getRedemptionAmount(token, _sgusdAmount);
    ...
    function getFullVaultTokenInfo(address _vault, address _weth, uint256 _sgusdAmount, address[] memory _tokens) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 6] = vault.maxSgusdAmounts(token);
    ...
    function getVaultTokenInfoV2(address _vault, address _weth, uint256 _sgusdAmount, address[] memory _tokens) public view returns (uint256[] memory) {
