...
contract EsSgxBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esSgx;

    constructor(address _esSgx) public {
        admin = msg.sender;
        esSgx = _esSgx;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsSgxBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esSgx).transferFrom(msg.sender, _accounts[i], _amounts[i]);
