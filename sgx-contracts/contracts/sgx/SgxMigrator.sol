...
import "./interfaces/ISgxIou.sol";
import "./interfaces/IAmmRouter.sol";
import "./interfaces/ISgxMigrator.sol";

contract SgxMigrator is ReentrancyGuard, ISgxMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "SgxMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "SgxMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "SgxMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "SgxMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "SgxMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "SgxMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "SgxMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "SgxMigrator: invalid _lpTokenBs.length");
    ...
    function migrate(
        ...
        require(isMigrationActive, "SgxMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "SgxMigrator: token not whitelisted");
        require(_tokenAmount > 0, "SgxMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "SgxMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(sgxPrice);
        require(mintAmount > 0, "SgxMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "SgxMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "SgxMigrator: invalid tokenA");
            require(tokenB != address(0), "SgxMigrator: invalid tokenB");
        ...
        ISgxIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "SgxMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "SgxMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "SgxMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "SgxMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("SgxMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "SgxMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "SgxMigrator: invalid _action");
