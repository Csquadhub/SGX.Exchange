...
const earliestTxn = 1659312000 // Monday, 1 August 2022 00:00:00
...
const distribute = async () => {
  ...
  const stakedSgxBalances = await processFile(inputDir + "staked-sgx-balances.csv")
  const vestedBalances = await processFile(inputDir + "vested-balances.csv")

  const holders = {}

  for (let i = 0; i < stakedSgxBalances.length; i++) {
    const tokenHolder = stakedSgxBalances[i]
  ...
  if (shouldSendTokens) {
    await sendTxn(esSgx.approve(batchSender.address, expandDecimals(totalEsSgx, 18)), "esSgx.approve")
  }
  ...
  for (let i = 0; i < balanceList.length; i++) {
    ...
    const esSgxValue = (totalEsSgx - 1) * balance / totalBalance
    const esSgxAmount = ethers.utils.parseUnits(esSgxValue.toFixed(4), 18)

    accounts.push(account)
    amounts.push(esSgxAmount)
    totalEsSgxAmount = totalEsSgxAmount.add(esSgxAmount)

    console.log(`${i+1},${account},${esSgxValue},${esSgxAmount.toString()}`)

    if (accounts.length === batchSize && shouldSendTokens) {
      console.log("sending batch", i, accounts.length, amounts.length)
      await sendTxn(batchSender.send(esSgx.address,  accounts, amounts), "batchSender.send")
  ...
  console.log("totalEsSgxAmount", totalEsSgxAmount.toString())
}
