...
async function readSgxLpManager(sgxlpManager) {
  console.log("sgxlpManager.cooldownDuration", (await sgxlpManager.cooldownDuration()).toString())
}
...
async function main() {
  ...
  // await getPool(weth.address, sgx.address, 10000)
  ...
  startTime = Date.now()
  await sgxlpManager.getAums()
  console.log("sgxlpManager.getAums", Date.now() - startTime)
}
