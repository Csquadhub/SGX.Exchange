...
async function getSgxPrice(ethPrice) {
...
async function getArbValues() {
  ...
  const stakedSgx = await contractAt("Token", "0xd2D1162512F927a7e282Ef43a362659E4F2a728F", signer)
  const stakedSgxSupply = await stakedSgx.totalSupply()
  const { totalTransferAmount: keeperCosts } = await getArbKeeperValues()
  const sgxlpManager = await contractAt("SgxLpManager", "0x321F653eED006AD1C29D174e17d96351BDe22649", signer)
  const sgxlpAum = await sgxlpManager.getAum(true)

  return { vault, reader, tokens, tokenInfo, nativeTokenPrice, feesUsd, stakedSgx, stakedSgxSupply, keeperCosts, sgxlpManager, sgxlpAum }
}
...
async function getAvaxValues() {
  ...
  const nativeTokenPrice = tokenInfo[tokens.nativeToken.address].maxPrice
  const feesUsd = await getFeesUsd(vault, reader, tokenInfo)
  const stakedSgx = await contractAt("Token", "0x4d268a7d4C16ceB5a606c173Bd974984343fea13", signer)
  const stakedSgxSupply = await stakedSgx.totalSupply()
  const { totalTransferAmount: keeperCosts } = await getAvaxKeeperValues()
  const sgxlpManager = await contractAt("SgxLpManager", "0xe1ae4d4b06A5Fe1fc288f6B4CD72f9F8323B107F", signer)
  const sgxlpAum = await sgxlpManager.getAum(true)
...
async function main() {
  ...
  const sgxPrice = await getSgxPrice(ethPrice)

  const data = [
    ["ETH Price", formatAmount(ethPrice, 30, 2)],
    ["AVAX Price", formatAmount(avaxPrice, 30, 2)],
    ["SGX Price", formatAmount(sgxPrice, 30, 2)],
    ["ARB Fees", formatAmount(values.arbitrum.feesUsd, 30, 2)],
    ["AVAX Fees", formatAmount(values.avax.feesUsd, 30, 2)],
    ["ARB sbfSGX", formatAmount(values.arbitrum.stakedSgxSupply, 18, 2)],
    ["AVAX sbfSGX", formatAmount(values.avax.stakedSgxSupply, 18, 2)],
    ...
    ["SGXLP AUM (ARB)", formatAmount(values.arbitrum.sgxlpAum, 30, 2)],
    ["SGXLP AUM (AVAX)", formatAmount(values.avax.sgxlpAum, 30, 2)],
  ]
