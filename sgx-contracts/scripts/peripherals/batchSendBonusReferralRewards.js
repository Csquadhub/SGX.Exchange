...
async function getArbValues() {
  ...
  return { batchSender, sgx, data, gasLimit, totalSgx, totalUsd }
}

async function getAvaxValues() {
  const batchSender = await contractAt("BatchSender", "0xF0f929162751DD723fBa5b86A9B3C88Dc1D4957b")
  const sgx = await contractAt("Token", "0x62edc0692BD897D2295872a9FFCac5425011c661")
  const data = avaxData
...
async function main() {
  ...
  const { batchSender, sgx, data, totalSgx, totalUsd, gasLimit } = await getValues()
  ...
  for (let i = 0; i < affiliatesData.length; i++) {
    ...
    const amount = bigNumberify(rebateUsd).mul(expandDecimals(totalSgx, 18)).div(expandDecimals(totalUsd, 30))
  ...
  if (shouldSendTxn) {
    ...
    await sendTxn(sgx.approve(batchSender.address, totalAmount, { gasLimit: 1000000 }), "sgx.approve")
    ...
    await processBatch([affiliateAccounts, affiliateAmounts], batchSize, async (currentBatch) => {
      ...
      await sendTxn(batchSender.sendAndEmit(sgx.address, accounts, amounts, affiliateRewardsTypeId, { gasLimit }), "batchSender.sendAndEmit(sgx, affiliate rewards)")
    })
  }
}
