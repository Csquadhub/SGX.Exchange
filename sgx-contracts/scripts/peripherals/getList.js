...
const holderList = require("../../data/holders/sgxHolders.json")
...
async function main() {
  ...
  for (let i = 3490; i < holderList.length; i++) {
    ...
    const bnSgxBalance = await feeSgxTracker.depositBalances(account, bnSgx.address)
    const pendingRewards = await bonusSgxTracker.claimable(account)
    const totalRewards = bnSgxBalance.add(pendingRewards)
    console.log(`${i+1},${account},${ethers.utils.formatUnits(bnSgxBalance, 18)},${ethers.utils.formatUnits(pendingRewards, 18)},${ethers.utils.formatUnits(totalRewards)}`)
