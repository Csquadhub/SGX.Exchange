...
async function main() {
  ...
  // const migrationTokens = ["GMT", "XGMT", "GMT_SGUSD", "XGMT_SGUSD"]

  const sgxMigrator = await contractAt("SgxMigrator", "0x0472F402EA8E301D7595545884Ad4C420E9865d6")
  const gmt = {
    name: "GMT",
  ...
  const tokens = [gmt, xgmt, gmtSgusd, xgmtSgusd]
  ...
  if (shouldApprove) {
    for (let i = 0; i < tokens.length; i++) {
      ...
      const migratedAmount = await sgxMigrator.migratedAmounts(account, token.contract.address)
      ...
      await sendTxn(sgxMigrator.setMaxMigrationAmount(account, token.contract.address, totalAmount), message)
    }
  }
}
