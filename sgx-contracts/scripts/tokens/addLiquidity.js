...
async function main() {
  ...
  const sgusd = await contractAt("SGUSD", "0xE14F46Ee1e23B68003bCED6D85465455a309dffF")
  ...
  await sendTxn(sgusd.approve(router.address, maxUint256), "sgusd.approve(router)")
  ...
  await sendTxn(router.addLiquidity(
    ...
    wallet.address, // to
    maxUint256 // deadline
  ), "router.addLiquidity(xgmt, sgusd)")
