...
const ARBITRUM_SUBGRAPH_ENDPOINT = 'https://api.thegraph.com/subgraphs/name/gdev8317/sgx-arbitrum-referrals-staging'
const AVALANCHE_SUBGRAPH_ENDPOINT = 'https://api.thegraph.com/subgraphs/name/gdev8317/sgx-avalanche-referrals-staging'

const BigNumber = ethers.BigNumber
const { formatUnits, parseUnits } = ethers.utils
const SHARE_DIVISOR = BigNumber.from("1000000000") // 1e9
const BONUS_TIER = 2 // for EsSGX distributions
const USD_DECIMALS = 30
const SGX_DECIMALS = 18
...
async function queryDistributionData(network, fromTimestamp, toTimestamp, account, sgxPrice, essgxRewards) {
  if (sgxPrice) {
    sgxPrice = parseUnits(sgxPrice, USD_DECIMALS)
  }
  if (essgxRewards) {
    essgxRewards = parseUnits(essgxRewards, SGX_DECIMALS)
  }
  ...
  if (sgxPrice && essgxRewards) {
    const essgxRewardsUsdLimit = essgxRewards.mul(sgxPrice).div(expandDecimals(1, SGX_DECIMALS))
    let essgxRewardsUsdTotal = BigNumber.from(0)
    Object.values(referrersRebatesData).forEach(data => {
      if (data.tierId !== BONUS_TIER) {
        return
      }
      data.essgxRewardsUsd = data.volume.div(1000).div(20) // 0.1% margin fee, 0.05% of fee is EsSGX bonus rewards
      data.essgxRewards = data.essgxRewardsUsd
        .mul(expandDecimals(1, USD_DECIMALS))
        .div(sgxPrice)
        .div(expandDecimals(1, 12))
      essgxRewardsUsdTotal = essgxRewardsUsdTotal.add(data.essgxRewardsUsd)
    })

    if (essgxRewardsUsdTotal.gt(essgxRewardsUsdLimit)) {
      const denominator = essgxRewardsUsdTotal.mul(USD_DECIMALS).div(essgxRewardsUsdLimit)
      Object.values(referrersRebatesData).forEach(data => {
        data.essgxRewardsUsd = data.essgxRewardsUsd.mul(USD_DECIMALS).div(denominator)
        data.essgxRewards = data.essgxRewardsUsd
          .mul(expandDecimals(1, USD_DECIMALS))
          .div(sgxPrice)
          .div(expandDecimals(1, 12))
  ...
  for (const data of Object.values(referrersRebatesData)) {
    ...
    consoleData.push({
      ...
      tierId: data.tierId,
      "essgxRewards, $": data.essgxRewardsUsd ? formatUnits(data.essgxRewardsUsd, USD_DECIMALS) : null,
      essgxRewards: data.essgxRewards ? formatUnits(data.essgxRewards, SGX_DECIMALS) : null,
    })
    output.referrers.push({
      ...
      tierId: data.tierId,
      essgxRewards: data.essgxRewards ? data.essgxRewards.toString() : null,
      essgxRewardsUsd: data.essgxRewardsUsd ? data.essgxRewardsUsd.toString() : null,
    })
  }
...
async function main() {
  ...
  parser.add_argument('-g', '--sgx-price', { help: 'SGX TWAP price' })
  parser.add_argument('-e', '--essgx-rewards', {
    help: 'Amount of EsSGX to distribute to Tier 3',
    default: "5000"
  })
